  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 14, minWidth: 220 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 4 }}>
        {DAYS.map(d => (
          <div key={d} style={{ fontSize: 10, color: "#555", textAlign: "center", padding: "2px 0" }}>{d}</div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 10, fontSize: 11, color: "#666" }}>
        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ width: 10, height: 10, borderRadius: 3, background: "rgba(108,99,255,0.3)", display: "inline-block" }} />
          Has data
        </span>
        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ width: 10, height: 10, borderRadius: 3, background: "#333", display: "inline-block" }} />
          No data
        </span>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    axios.get(`${API}/dates`)
      .then(r => {
        setDates(r.data);
        if (r.data.length > 0) setSelected(r.data[0].reading_date);
      })
  ...
  const fetchData = useCallback(async (overrideDate) => {
    ...
    try {
      ...
      const [curveRes, summaryRes] = await Promise.all([
        axios.get(url),
        mode === "single"
          ? axios.get(`${API}/summary?date=${dateToFetch}`)
  ...
  function handleSingleDateChange(iso) {
    setSelected(iso);
  }
  ...
  const StatCard = ({ label, value, unit, highlight }) => (
    <div style={{
      ...
      <div style={{ fontSize: 20, fontWeight: 600, color: highlight ? "#a99fff" : "#fff" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d12", color: "#fff", fontFamily: "system-ui, -apple-system, sans-serif", paddingBottom: "3rem" }}>
      ...
      <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid rgba(255,255,255,0.07)", marginBottom: "1.5rem" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap", marginBottom: "1rem" }}>
            <h1 style={{ fontSize: 19, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>NLDC Grid Dashboard</h1>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 1.5rem" }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
          ...
          {/* Summary cards — sit next to calendar */}
          {summary && (
        ...
        {error && (
          <div style={{
            padding: "10px 14px", background: "rgba(220,50,50,0.12)",
            border: "1px solid rgba(220,50,50,0.3)", borderRadius: 8,
          ...
          <div style={{
            ...
            <ResponsiveContainer width="100%" height={400}>
              <ComposedChart data={data} margin={{ top: 4, right: 20, left: 4, bottom: 4 }}>
                ...
                <Brush dataKey="time_slot" height={26}
                  stroke="rgba(108,99,255,0.35)" fill="rgba(108,99,255,0.06)"
                  travellerWidth={6} tickFormatter={tickLbl} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        )}
